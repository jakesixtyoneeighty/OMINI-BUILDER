...
export const AssistantMessage = memo(({ content, tokenUsage, isStreaming }: AssistantMessageProps) => {
  const { thinkMode } = useStore(chatStore);
  ...
  // Show thinking block when:
  // - thinkMode is active AND we're streaming (show "Pensando..." immediately)
  // - OR there's actual thinking content
  // - OR an unclosed <think> tag is detected (streaming in progress)
  const showThinking = (thinkMode && isStreaming) || !!thinking || isThinking;

  // Effective streaming state for ThinkingBlock:
  // - isThinking (unclosed tag) = definitely still streaming
  // - thinkMode + isStreaming + no content yet = show initial "Pensando..." animation
  const effectiveIsThinking = isThinking || (!!thinkMode && !!isStreaming && !thinking);

  if (!displayContent && !thinking && !(thinkMode && isStreaming)) {
    return null;
  }

  return (
    <div className="overflow-hidden w-full">
      {showThinking && <ThinkingBlock content={thinking} isStreaming={effectiveIsThinking} />}
